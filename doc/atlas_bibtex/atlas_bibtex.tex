...
\item[@online] Use it for things that are only available online,
  e.g.~\cite{lshort}.
  If you use traditional \BibTeX you have to use \Macro{@misc} instead.
...
\item[\Option{showdoi=true|false}] make the journal reference a link to the DOI (default) instead of displaying it;
\item[\Option{eprint=true|false}] print the arXiv reference if available (default).
\item[\Option{address=true|false}] turn on or off (default) the printing of the address field.
\item[\Option{location=true|false}] turn on or off (default) the printing of the location field.
