...
\item[floatopt] adjust the options governing number and placement of floats on a page;
...
\item[newtx] use the \Package{newtxtext} and \Package{newtxmath} packages for fonts (default);
...
\item[\Option{texlive=2009}] set if you use an older version of \TeX\ Live like 2009.
...
\Package{newtx} fonts. 
Hence, in ATLAS papers, please add the option \Option{txfonts} to \Package{atlasdoc}.
As soon as arXiv switches to a newer version of \TeX\ Live this option will no longer be necessary.
...
You also have to add the option \Option{texlive=2009} to \Package{atlaspackage} and \Package{biblatex}
(if you are using \Package{biblatex}).
If you are using \Package{siunitx}, then you also have to load \Package{atlasphysics}
with the option \Option{unit=off}.
It is also possible to pass the option \Option{texlive=2009} to the main class \Package{atlasdoc}.
The following extra packages had to be installed:
\begin{itemize}\setlength{\parskip}{0pt}\setlength{\itemsep}{0pt}
\item \Package{texlive-latex-extra} (footmisc);
\item \Package{texlive-humanities} (lineno);
...
\item \Package{texlive-newtx}, \Package{texlive-newtx-doc} and \Package{texlive-boondox} (newtx);
...
\item \Package{texlive-mhchem} (mhchem);
\item \Package{texlive-elsarticle} (elsarticle) Elsevier journals;
\item \Package{texlive-revtex} (revtex) APS journals;
\end{itemize}
...
If you want to use the ATLAS preprint style you also have to install
\Package{newtx}, \Package{fontaxes} and \Package{boondox}.
For the documentation and the heavy ion symbols you also need the \Package{mhchem} package.
...
If you download the packages from CTAN, follow the \File{README} in each package to install them.
The \Package{boondox} packages also needs \File{MD5.pm} in order to update the font map.
You can install it with the command \texttt{yum install perl-Digest-MD5} as superuser.
