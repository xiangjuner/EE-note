...
The selected \TeX\ Live version is set with the \Option{TEXLIVE} option. The default is 2020.
...
\item[\Option{texlive=2020}] adjust if you use an older version of \TeX\ Live like 2013.
...
Do not forget to adjust \Macro{author} accordingly,
...
\ADOCnew{12.1} \Package{diffcoeff} will not be included for \TeX\ Live versions older than 2016 (defined by the \Option{texlive} option).
...
\item[\Option{texlive=2020}] set if you use an older version of \TeX\ Live like 2013.
  % A value less than 2013 adjusts a few \Package{biblatex} options.
...
At the same time you should pass the option \Option{texlive=2020} to
\Package{atlasdoc}.
...
\pdfoutput=1
\RequirePackage{latex/atlaslatexpath}
\documentclass[cernpreprint, atlasdraft=false, texlive=2020, UKenglish]{atlasdoc}
\end{tcblisting}
...
\begin{description}
  ...
  \ADOCnew{12.0.0} The package is included if you pass the option \Option{full} to \Package{atlaspackage}.
  If the document language is set to \Option{USenglish},
  a slanted \textit{d} is used in differentials,
  while an upright \textrm{d} is used for other languages.
  I recommend \Macro{dl2x} to get a nice spacing for the differential in an integral,\\
  % diffcoeff tweaks for older TeX Live versions.
  \ifthenelse{\AtlasTeXLiveVersion < 2018}{%
    \emph{Examples are not complete as \Package{diffcoeff} package not available for older versions of TeX Live.\\}}{}
  e.g.\ \verb|\int f(x) \dl2x| gives 
  \ifthenelse{\AtlasTeXLiveVersion < 2018}{XXX, while}{\(\int f(x) \dl2x\), while}\\
  \verb|\int f(x) \dl0x| or \verb|\int f(x) \dl x| gives  
  \ifthenelse{\AtlasTeXLiveVersion < 2018}{XXX.}{\(\int f(x) \dl0x\).}
