...
The selected \TeX\ Live version is set with the \Option{TEXLIVE} option. The default is 2013.
...
As mentioned above the default \TeX\ Live version is currently set to 2013.
...
To test compatibility with arXiv, try a setting of 2011.
...
\item[NOTE] the document is an ATLAS note (default);
\item[PAPER] the document is an ATLAS paper (draft);
...
\item[maketitle=true|false] turn on or off (default) the creation of a title page within \Macro{AtBeginDocument}.
  If the option is false, but you still want a title, give the command \Macro{maketitle} after
  \verb|\begin{document}|. This is the recommended way of proceeding;
\item[nomaketitle] do not create a title page; equivalent to \Option{maketitle=false}.
  This option has no effect as of Version 01-07-00 of \Package{atlaslatex};
\item[floatopt=true|false] adjust the options (default) governing number and placement of floats on a page;
...
\item[newtx] use the \Package{newtxtext} and \Package{newtxmath} packages for fonts 
...
\item[\Option{biblatex=true|false}] (false) turn on/off use of \Package{biblatex} and \Package{biber/bibtex};
...
\item[\Option{maxbibnames=n}] (3) specify the maximum number of author names before \enquote{et al.} is used, where n is an integer;
...
\item[\Option{eVkern=true|false}] (false) apply a kern of -0.1em to \si{\eV} in order to move \enquote{e} and \enquote{V} closer together;
...
\item[\Option{texlive=YYYY}] set if you use an older version of \TeX\ Live like 2009.
  ...
  Use \Option{texlive=2011} to test options for arXiv submissions;
...
\Package{newtx} fonts. 
Hence, in ATLAS papers, please add the option \Option{texlive=2011} to \Package{atlasdoc}.
...
At the same time you should pass the option \Option{texlive=2011} (or earlier) to
\Package{atlasdoc}. This is because arXiv contains a 2011 \TeX\ Live installation.
Summarising, a typical \Macro{documentclass} command would be:
\begin{verbatim}
\documentclass[cernpreprint,texlive=2011,txfonts,UKenglish]{latex/atlasdoc}
\pdfoutput=1
\end{verbatim}
...
You can test whether your document compiles with \TeX\ Live 2011 by setting your \texttt{PATH} on \texttt{lxplus} to
\begin{verbatim}
...
\File{latex}, \File{logos} and \File{bibtex} directories of the \Package{atlaslatex} package to SVN.
...
\verb|\newcommand*{\ATLASLATEXPATH}{../atlaslatex-01-06-01/}|
to use version 01-06-01 of the \Package{atlaslatex} package, which you have unpacked into the directory tree
\File{../atlaslatex-01-06-01}.
...
Hence, for CONF and PUB notes it is important to comment out \Macro{AtlaVersion} in order to suppress the line numbers for the final version.
...
\verb|\sisetup{separate-uncertainty, range-units=repeat, detect-all}|
and\\
