...
The package should work for \TeX\ Live versions from 2013 onwards\ADOCnew{09-00-00}.
...
\item[\Option{mhchem={\normalfont true}|false}] include \Package{mhchem} before \Package{xpatch} to avoid
  ...
  \emph{This option has no effect as of \Package{atlaslatex} 07-00-00};
...
The option \Option{atlasdraft} was introduced with \Package{atlaslatex} 04-03-00\ADOCnew{04-03-00}.
It is intended to replace using the definition of \Macro{AtlasVersion} to decide if this a final version of a document.
...
\item[nomaketitle] do not create a title page; equivalent to \Option{maketitle=false}.
  \emph{This option has no effect as of \Package{atlaslatex} 01-07-00 of \Package{atlaslatex}};
...
\item[\Option{letterpaper}] set the paper size to letter.
  \emph{This option does not work as of \Package{atlaslatex} 07-00-00
...
If you cannot find it, it is also included in the \File{obsolete} directory.
...
\File{atlaslatex-XX-YY-ZZ.tds.zip} file into your \File{texmf} tree --- see \cref{sec:texmf}.
...
For example:
\begin{tcblisting}{listing only}
\ATLnote{JS}{Jane}\ATLnote{JS}{Jane}{magenta!20}{magenta}
\end{tcblisting}
...
Here is an \JSnote{example paragpraph}{Illustration of macro}
that shows how to use the macro.It you want a comment,
...
add \Macro{listtodonotes} somewhere in your document.
...
As of Version 06-00-00 of \Package{atlaslatex}
...
As of Version 08-04-00 of \Package{atlaslatex} line numbering around AMS Math \LaTeX\ environments such as \Package{align}
should just work.
