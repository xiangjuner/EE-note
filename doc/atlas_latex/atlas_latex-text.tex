...
\item[a4paper] set the paper size to A4 (default);
  This option was removed in \Package{atlaslatex} 02-00-00;
...
\item[BOOK] the document is book form like an LOI or TDR;
\item[auxmat] the document contains auxiliary material for a preprint;
\item[maketitle={\normalfont true}|false] turn on or off (default) the creation of a title page within \Macro{AtBeginDocument}.
  ...
  \emph{This option is only available in \Package{atlasdoc1} as of \Package{atlaslatex} 02-00-00.}
...
\item[titleextraheight=0em] extra height that could be needed for the title, if it is very long;
\item[copyrightoffset=0em] adjust the position of the copyright,
...
At the same time, the option BOOK was introduced, which user \Package{scrbook} as the base class.
...
This class (\Package{atlasdoc1}) will not be actively maintained or developed any more.
