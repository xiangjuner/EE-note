...
\begin{description}
  ...
  \ifthenelse{\AtlasTeXLiveVersion > 2018}{\(\int f(x) \dl2{x}\)}{XXX},
  while \verb|\int f(x) \dl0{x}| or \verb|\int f(x) \dl x| gives  
  \ifthenelse{\AtlasTeXLiveVersion > 2018}{\(\int f(x) \dl0{x}\).}{XXX.\\
  ...
  \Option{diffcoeff} to \Package{atlaspackage} and include the package yourself using
  with the command \Macro{usepackage\{diffcoeff\}[=v4]}.
