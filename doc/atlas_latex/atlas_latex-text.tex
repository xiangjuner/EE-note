...
\emph{\ADOCold{14.0.0} This \Option{TEXLIVE} option is obsolete and ignored.}
...
\begin{description}
  ...
  \ADOCnew{14.0.0} The package is included if you pass the option \Option{full} to \Package{atlaspackage}.
  ...
  \makeatletter
  \@ifpackagelater{diffcoeff}{2023/01/01}{%
    In version 5 (released in 2023), the \Macro{dl} command includes a small space before the differential,
    e.g.\ \(\int f(x) \dl{x}\).
    In older versions one has to use a syntax like \Macro{dl2\{x\}}.
  }{%
    I reccommend \Macro{dl2\{x\}} to get a nice spacing for the differential in an integral,\\
    e.g.\ \verb|\int f(x) \dl2{x}| gives
    \@ifpackagelater{diffcoeff}{2018/04/01}{\(\int f(x) \dl2{x}\)}{XXX},
    while
    \verb|\int f(x) \dl0{x}| or \verb|\int f(x) \dl x| gives  
    \@ifpackagelater{diffcoeff}{2018/04/01}{\(\int f(x) \dl0{x}\).}{XXX.\\
