...
The \Package{tabular} environment does not like this. Add the option \Option{xtab} to use 
\Package{xtab} instead.
...
\item \texttt{xtab} Newer version of supertabular, which allows tabular environments to go over mote than one page;
\item \texttt{rotating} Rotate figures, tables, etc.;
\item \texttt{xfrac} Nicer fractions.
\end{itemize}

\texttt{lineno.sty} and \texttt{authblk.sty} are also provided in \texttt{latex/other}.
...
for the CERN preprint version of an ATLAS paper. The style can be found in \texttt{latex/atlascover.sty}.
...
\item[twocolumn] Use this option if your document is formatted in two columns;
\item[atlasdoc] This option is set by \Package{atlasdoc} and is not usually needed by the user.
\end{description}
...
\Package{atlascover} needs the package \Package{kvoptions} so that it can process
options where a value for an option should be set.
...
\Package{texlive-booktabs} (booktabs),
\Package{texlive-preprint} (authblk),
\Package{texlive-tocloft} (tocloft),
\Package{texlive-xtab} (xtab),
\Package{texlive-placeins} (placeins).
...
\item[APS] APS journals use REV\TeX. This is also usually installed.
	...
	Hence you should comment out the definition in your metadata file, e.g.\ \File{mydocument-metadata.tex}.
...
In general you should separate the figure and table environments from the text by blank lines.
