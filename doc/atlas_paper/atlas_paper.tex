...
%-------------------------------------------------------------------------------
\documentclass[PAPER,UKenglish,texlive=2013]{latex/atlasdoc}
...
which is consistent with the standard model prediction of \SI[trapambigerr=false]{44(3)}{\pb} calculated at next-to-leading order in QCD. 
}{%
\SI[parse-numbers=false]{41^{+20}_{-16}(\text{stat.})\pm 5(\text{syst.})\pm 1(\text{lumi.})}{\pb},
which is consistent with the standard model prediction of \SI[multi-part-units=single]{44 \pm 3}{\pb} calculated at next-to-leading order in QCD. 
}
